...
async function grabChildren(account) {
    ...
    let before = "";
    while (true) {
        let url = `${hyperionBaseURL}?account=${account}&limit=1000&act.name=newaccount${before === "" ? "" : `&before=${before}`}`;
        let actions = await getActions(url);

        if (actions.length === 1 && children.includes(actions[0].act.data.newact))
            break;

        const newAccounts = actions.map(function(it){return it.act.data.newact});

        before = actions[actions.length-1].timestamp;
        children = children.concat(newAccounts);
        console.log("Grabbed:", newAccounts.length, "Total", children.length, newAccounts[0], newAccounts[newAccounts.length-1]);
